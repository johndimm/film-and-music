...
export default function SettingsPage() {
      ...
      try {
        ...
        setImportConfirm({
          ...
          apply: () => {
            ...
            window.location.href = "/trailer-visions";
          },
        });
      } catch {
  ...
  return (
    ...
    <ConfirmDialog
      ...
      onConfirm={() => {
        setResetOpen(false);
        wipeTrailerVisionStorageAndApplyFactory();
        window.location.replace("/trailer-visions");
      }}
    >
      Wipe this app&apos;s stored data in this browser, then re-load the same bundled channels,
      prefetch queues, and active channel as on a true first open (the factory-channels.json
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm divide-y divide-zinc-100">
          <div className="px-4 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex-1">
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Wipe all Trailer Vision storage and re-seed the factory starter (same as empty localStorage
                on first open). Use this to test a &quot;new user&quot; without DevTools. Cannot be undone.
              </p>
            </div>
            <button
              type="button"
