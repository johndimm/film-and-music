...
import { trailerVisionStorage, trailerVisionChannelIds } from "@/app/lib/platform";
...
const ARTIST_SUGGEST_CACHE_STORAGE = trailerVisionStorage.artistSuggestionsV1;
...
export default function ChannelsPage() {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      try {
        ...
        if (!chs.find((c) => c.id === trailerVisionChannelIds.all)) {
          chs = [ALL_CHANNEL, ...chs];
  ...
  useEffect(() => {
    ...
    window.history.replaceState({}, "", "/trailer-visions/channels");
  ...
  const saveChannels = (chs: Channel[]) => {
    ...
    const withAll = chs.find((c) => c.id === trailerVisionChannelIds.all) ? chs : [ALL_CHANNEL, ...chs];
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          {!showNew && selected && (
            <div className="p-4 sm:p-6 space-y-6">
              ...
              {/* Edit form */}
              {selected.id !== trailerVisionChannelIds.all && (
                <div>
