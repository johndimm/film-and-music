...
const PlayerConstellationsEmbed = dynamic(() => import('./PlayerConstellationsEmbed'), { ssr: false })
...
export default function PlayerClient({
  ...
  youtubeOnly?: boolean
  /** Film & music unified shell embeds Soundings in an iframe; skip duplicate top nav. */
  ...
  useEffect(() => {
    try {
      const v = localStorage.getItem(soundingsStorage.autoNextAtEnd)
      if (v === '1') {
  ...
  const createChannelWithNotes = useCallback(async (notes: string, opts?: { name?: string }) => {
    if (channelSwitchingRef.current) return
    const trimmed = notes.trim()
    const fromOpts = (opts?.name ?? '').replace(/\s+/g, ' ').trim()
    const name =
      fromOpts.length > 0
        ? fromOpts.slice(0, 80)
        : (() => {
  ...
  const newChannelFromConstellationsNode = useCallback(
    (node: GraphNode) => {
      const { name, notes } = graphNodeToChannelSeeds(node)
      void createChannelWithNotes(notes, { name })
    },
    [createChannelWithNotes]
  )

  /** /constellations full screen queues notes here; we create the channel once /player is shown. */
  useEffect(() => {
    if (!pathname?.startsWith('/player')) return
    const key = soundingsStorage.pendingConstellationsNewChannel
    ...
    void createChannelWithNotes(notes, nameOpt ? { name: nameOpt } : undefined)
  }, [pathname, createChannelWithNotes])
  ...
  const enterCareerMode = useCallback(async (artistName: string) => {
    ...
    try {
      ...
      const res = await fetch(`${CAREER_API.musicDiscography}?${params}`, { credentials: 'same-origin' })
      ...
      const works: MusicCareerWork[] = data.works ?? []
      ...
      const cm: MusicCareerMode = { artistName, works, currentIndex }
      setCareerMode(cm)
  ...
  useEffect(() => {
    ...
    const clearPendingShare = () => {
      try {
        sessionStorage.removeItem(soundingsStorage.pendingShare)
      } catch {}
    }
   ...
   * CustomEvent {@link soundingsEventNames.enqueue} with an ordered array of CardStates when the
  ...
  useEffect(() => {
    ...
    window.addEventListener(soundingsEventNames.enqueue, handler as EventListener)
    return () => window.removeEventListener(soundingsEventNames.enqueue, handler as EventListener)
  }, [isGuideDemo])
    ...
    <div data-guide="full-player" className="min-h-screen min-w-[min(100%,900px)] bg-black text-white flex flex-col overflow-x-hidden">
      ...
      <div className="flex flex-col items-center gap-3 p-4 overflow-y-auto flex-1">
        ...
        <div
          ...
          className="relative rounded-xl overflow-hidden w-full aspect-[4/3] bg-slate-900"
            ...
            <label className="flex items-center gap-2 text-xs text-zinc-500 cursor-pointer select-none pl-0.5">
              <input
                ...
                onChange={e => {
                  ...
                  try {
                    localStorage.setItem(soundingsStorage.autoNextAtEnd, v ? '1' : '0')
                  } catch {}
                }}
              />
        ...
        <div
          data-guide="sidebar"
          className="w-full p-4 rounded-xl border border-slate-700 bg-slate-900/95 backdrop-blur-xl shadow-2xl relative overflow-hidden flex flex-col"
        >
          <div className="min-h-0 max-h-[min(70vh,32rem)] overflow-y-auto overflow-x-hidden pr-1 -mr-1 custom-scrollbar">
          <SessionPanel
            queue={queue}
      ...
      {pathname.startsWith('/player') && !hideAppChrome ? (
        <PlayerConstellationsEmbed onNewChannelFromNode={newChannelFromConstellationsNode} />
      ) : null}
