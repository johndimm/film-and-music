...
export interface NextMovieResponse {
  ...
  rtScore: string | null;
  reason: string | null;
  /** US streaming service names from the same LLM call (movies and TV series). May be incomplete or wrong — app may refine later when empty. */
  streaming: string[];
}

/** After Serper returns quota / credits error, skip further image calls (process lifetime). */
let serperPosterDisabled = false;
let serperCreditsWarningLogged = false;

function serperErrorLooksLikeExhausted(status: number, body: string): boolean {
  const b = body.toLowerCase();
  if (status === 402) return true;
  if (status === 400 && (b.includes("credit") || b.includes("quota") || b.includes("not enough"))) return true;
  if (b.includes("not enough credits")) return true;
  return false;
}

function markSerperExhausted(): void {
  serperPosterDisabled = true;
  if (serperCreditsWarningLogged) return;
  serperCreditsWarningLogged = true;
  console.warn(
    "[next-movie] Serper Images API quota/credits exhausted — poster image fallback disabled for this process. " +
      "Add credits at https://serper.dev or set TMDB_API_KEY (recommended) for posters and trailers."
  );
}
...
interface RawItem {
  ...
  reason?: string | null;
  /** US streaming options for the movie or TV show (whole series level for type "tv"); same-call estimate. */
...
/** Slightly elevated to fit streaming_services arrays per title. */
...
async function fetchPosterFromSerper(title: string, type: "movie" | "tv", year: number | null): Promise<string | null> {
  ...
  if (serperPosterDisabled) return null;
  ...
  try {
    ...
    if (!res.ok) {
      const errBody = await res.text().catch(() => "");
      if (serperErrorLooksLikeExhausted(res.status, errBody)) {
        markSerperExhausted();
        return null;
      }
...
export async function POST(request: Request) {
  ...
  const informativeHistory = selectInformativeByDivergence({
    items: history,
    maxEntries: MAX_HISTORY_DIVERGENCE_LINES,
    getKey: (e) => e.title.toLowerCase(),
    getDivergence: movieDivergenceScore,
