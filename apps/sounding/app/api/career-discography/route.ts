  ...
  try {
    ...
    if (!res.ok) {
      ...
      return Response.json(
        { error: msg, works: [] as MusicCareerWork[] },
        { status: 500 }
      )
    }
    ...
    if (!raw) {
      ...
      return Response.json(
        { error: 'Empty LLM response', works: [] as MusicCareerWork[] },
        { status: 500 }
      )
    }
  } catch (e) {
    ...
    return Response.json(
      { error: 'LLM request failed', message: msg, works: [] as MusicCareerWork[] },
      { status: 500 }
    )
  }

  let works: MusicCareerWork[] = []
  const jsonSlice =
    raw.match(/```(?:json)?\s*(\[[\s\S]*\])\s*```/i)?.[1] ?? raw.match(/\[[\s\S]*\]/)?.[0] ?? null
  try {
    if (jsonSlice) works = JSON.parse(jsonSlice) as MusicCareerWork[]
  } catch (e) {
    console.error('[career-discography] JSON parse', e, 'raw start:', raw.slice(0, 500))
    return Response.json(
      { error: 'Failed to parse LLM response', works: [] as MusicCareerWork[] },
      { status: 500 }
    )
  }
  if (!Array.isArray(works)) {
    return Response.json(
      { error: 'LLM did not return a JSON array', works: [] as MusicCareerWork[] },
      { status: 500 }
    )
  }
