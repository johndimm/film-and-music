    ...
    <div className="min-h-screen bg-white text-black flex flex-col">
      ...
      <div className="flex flex-1 min-h-0 flex-col sm:flex-row gap-0 max-w-[800px] mx-auto w-full">
        ...
        <div className="shrink-0 sm:w-48 m-2 mb-0 sm:m-0 flex flex-col min-h-0 sm:max-h-full rounded-xl sm:rounded-none border border-slate-700 sm:border-b-0 sm:border-t-0 sm:border-l-0 bg-slate-900/95 backdrop-blur-xl shadow-2xl sm:shadow-none sm:backdrop-blur-none p-3 sm:border-r sm:border-slate-700">
          ...
          <div className="flex flex-col gap-1 min-h-0 max-h-[min(42svh,16rem)] sm:max-h-full sm:flex-1 overflow-y-auto pr-1 -mr-1 custom-scrollbar">
          {channels.map(c => (
            <div key={c.id} className="flex items-center gap-1 group">
              {editingChannelId === c.id ? (
                <input
                  className="flex-1 bg-slate-800/80 outline-none text-xs text-slate-100 border border-slate-600 rounded px-1.5 py-0.5 focus:border-indigo-500/50"
                ...
                <button
                  ...
                  className={`flex-1 text-left text-xs px-2 py-1.5 rounded-lg transition-colors truncate ${
                    c.id === activeChannelId
                      ? 'bg-slate-800 text-white font-medium border border-slate-600/60'
              ...
              {channels.length > 1 && c.id !== ALL_CHANNEL_ID && (
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-400 text-xs transition-all px-1"
