...
export async function POST(request: Request) {
  const { title, year, llm = "deepseek", sessionId }: { title: string; year: number | null; llm: string; sessionId?: string } =
    await request.json();
  ...
  try {
    ...
    const userKey = sessionId ? `tv-${sessionId}` : "tv-anon";
