/**
 * Merge seen ratings, blue-star (unseen) events, and pass-without-rating into one timeline
 * for the History page. Safe to import from client components only when reading localStorage.
 */
...
export type PresentationHistorySeen = {
  ...
  /** ISO time when the user submitted a red-star rating (newer clients). */
  ratedAt?: string
}
...
/** Below real ISO timestamps so unknown-time rows sort after dated ones when using descending sort. */
...
/**
 * Accepts legacy `string[]` or `{ title, at?, channelId? }[]`.
 */
...
/**
 * Stable list of everything the user was shown and then advanced past (rated or not).
 * Newest-first when using `sortMs` descending.
 */
